...
from consol.ingestion.validators import normalize_caption, require_columns, to_numeric
...
def load_ic(source: str | Path | BinaryIO | bytes, filename: str | None = None) -> pd.DataFrame:
    ...
    out["caption"] = normalize_caption(df["caption"]) if "caption" in df.columns else ""
